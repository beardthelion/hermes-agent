...
3. `/memory` upstream PR — **in progress.** Local branch `feat/native-memory-slash-command` off `origin/main` (tip `3034eee38`). Plan at ~/.hermes/plans/native-memory-slash-command-pr1.md. Two PRs: PR1 (CLI + gateway + core API) off origin/main, PR2 (api_server handler) off branch with #17178. **Tasks done:** 0 (branch setup), 1 (`MemoryStore.get_readout` — commit `9e440edab`), 2 (`parse_memory_command` — commit `753d0c33d`). **Next:** Task 3 — create `tools/memory_format.py` with three platform formatters (`format_memory_cli`, `format_memory_markdown`, `format_memory_plain`). Smoke tests on Tasks 1-2 confirmed real-data readout (6 memory/8 user entries, ~89%/91% capacity) and 15-case arg-parse coverage. Build-then-decide-on-PR — not yet pushed to fork. Decision on upstream submission deferred until after local dogfood.
