...
3. `/memory` upstream PR — **PR1 filed as #27207 (2026-05-17), awaiting maintainer review.** Branch `feat/native-memory-slash-command` rebased onto current `origin/main`, pushed to fork. 9 commits, +481/-2 across 8 files. Both follow-up fixes folded in (rebased commit IDs: `1d0f3618c` Sendblue plain-formatter routing; `d02437d64` `strip_markdown` regex tightening). PR body filled per `.github/PULL_REQUEST_TEMPLATE.md`; 36 memory-targeted tests cited green under `scripts/run_tests.sh`. Plan at ~/.hermes/plans/native-memory-slash-command-pr1.md. PR2 (api_server handler) still pending — needs #17178 merged first. **Harness gap closed**: `.venv` had no `pip`/`pytest-xdist`; bootstrapped via `python -m ensurepip` + `pip install pytest-xdist`. Don't merge #27207 into production locally — rebases on maintainer feedback would invalidate any local merge (production already carries the same commits via the original feature path). **Regex-tightening commit `d02437d64` superseded upstream (2026-05-17 update)**: Ambuj Kumar's `a3017508b` ("fix(gateway): preserve underscores in plain-text identifiers", merged 2026-05-16) tightened the same `_RE_BOLD_UNDER` / `_RE_ITALIC_UNDER` patterns one day before #27207 was filed. Production took upstream's version in merge commit `cc0a64853` (the resolution differs from `d02437d64`: upstream uses `\b__(?![\s_])(.+?)(?<![\s_])__\b` allowing underscores inside the bold span; ours used `[^_]+?` rejecting them entirely). When rebasing #27207 onto current `origin/main`, expect `d02437d64` to conflict with `a3017508b` — likely the right call is to drop `d02437d64` from the PR (upstream already solved it) unless our stricter `[^_]+?` semantics are demonstrably needed for /memory output.
