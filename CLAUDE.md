...
3. `/memory` upstream PR — **PR1 feature-complete, merged to production for local dogfood (2026-05-16).** Branch `feat/native-memory-slash-command` off `origin/main` (tip `3034eee38`). Plan at ~/.hermes/plans/native-memory-slash-command-pr1.md. Two PRs: PR1 (CLI + gateway + core API) off origin/main, PR2 (api_server handler) off branch with #17178. **Tasks done:** 0–7 of 8. Commits: `9e440edab` (`MemoryStore.get_readout`), `753d0c33d` (`parse_memory_command`), `72bf46039` (`tools/memory_format.py` — Rich/markdown/plain), `4b4235d03` (`CommandDef` registration), `74529a611` (CLI handler), `8408d716f` (gateway handler), `b423985df` (36 tests passing via direct pytest). **Task 8 (push/PR decision):** held pending dogfood. Branch lives locally only; not pushed to fork yet. Future upstream PR will rebase off origin/main and may invalidate the local production merge — re-merge after rebase. CI-harness verification (`scripts/run_tests.sh`) needs pytest-xdist installed in `.venv` before upstream submission.
