...
- **Onboarding flow for non-Mac users (implemented 2026-05-20, awaiting validation 3+, NOT pushed)** — design at `~/.hermes/plans/sendblue-onboarding-design.md`. Branch `feat/sendblue-onboarding` off `origin/main`, **9 commits, +651 lines, 23 tests green** under `scripts/run_tests.sh`. Tip `a1d97f5ac`. Commit chain (oldest first): `81a40da16` toolsets.py composite + gateway aggregate → `880cbf02e` PlatformInfo entry → `1d7c1e456` _PLATFORMS entry → `08be5545d` _setup_sendblue + 4 helpers + dispatch → `3a9b1204d` missing-home warning → `e9e43c478` 23 unit tests → `ef4dd3648` docs page + index updates → `e2eb7f28d` real /api/account/webhooks endpoint + KID-only-auth honesty → `a1d97f5ac` column-0 paste-safe snippets. **Live testing turned up two corrections to the plan**: (a) plan's `/accounts/me` endpoint is fictional — real read endpoint is `GET /api/account/webhooks`, verified live against api.sendblue.com 2026-05-20; (b) Sendblue's GET endpoints only check the API key ID — any non-empty secret value returns 200 OK with real data, so setup validation catches typo'd KIDs but a typo'd secret only surfaces at first-connect when the adapter POSTs to register the webhook. Both quirks now documented in the validator docstring, user-facing print ("API key ID verified (secret will be checked at first connect)"), and the docs troubleshooting section. Cloudflare-UA memory also re-confirmed: urllib default UA → HTTP 403 error 1010, custom UA → 200. **Validations done locally**: (1) menu surfacing — Sendblue at position 15/23, renders `📨 Sendblue (iMessage, no Mac)`, status flips configured/not-configured against HERMES_HOME; (2) snippet rendering — tunnel guidance prints all 4 options with correct anchors, reverse-proxy snippets extract hostname correctly and now print column-0 for safe Caddyfile paste. **Validations pending** before push/PR: (3) end-to-end wizard run with `HERMES_HOME=/tmp/sendblue-onboarding-test` and real creds (interactive, mutates env), (4) negative paths (empty URL → guidance + bail; bad KID → 401 + retry), (5) reconfigure path. Deferred per plan section 10: API-driven number selection, clipboard-copy of generated secret, config.yaml-to-env migration tooling, separate tunnel-option doc pages (rolled into one big page with anchors instead). DO NOT merge this branch into production locally — it's an upstream-PR branch; rebases on maintainer feedback would invalidate any local merge.
